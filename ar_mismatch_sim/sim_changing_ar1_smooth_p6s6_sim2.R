...
for (j in 1:reps_per_job) {
    ...
    na_inds <- sample(1:length(obs_data), 1/5*length(obs_data))
    obs_data[na_inds] <- NA
    resVAEar1_r <- iVAEar_radial(obs_data, coords_time[, 1:2],
        ...
        epochs = epochs, validation_split = 0, initial_batch_size = 64, seed = seed
    )
    cormat2 <- cor(resVAEar1_r$IC, data_scaled)
    if (anyNA(cormat2)) {
...
save(res_test, file = paste0("res_ar1_smooth_sim2/", filename))
res_test
