...
for (j in 1:reps_per_job) {
    ...
    ar2_scale_params <- runif(p, 1, 10)
    ar3_scale_params <- runif(p, 1, 10)
    ...
    ar1_params_arr <- array(NA, c(n_t, n_s, p))
    ar2_params_arr <- array(NA, c(n_t, n_s, p))
    ar3_params_arr <- array(NA, c(n_t, n_s, p))
    for (i in 1:p) {
        coefs <- runif(p)
        shift_covmat <- matern_covmat(coords, shift_params[[i]]$range, shift_params[[i]]$nu)
        shift_params_space1 <- t(rnorm(n_s, 0, 0.3)) %*% chol(shift_covmat)
        shift_params_space2 <- t(rnorm(n_s, 0, 0.3)) %*% chol(shift_covmat)
        shift_params_space3 <- t(rnorm(n_s, 0, 0.3)) %*% chol(shift_covmat)
        for (t_i in 1:n_t) {
            ar1_params <- coefs[1] * cos((2 * pi * rep(t_i, n_s) * ar1_scale_params[i]) / n_t - shift_params_space1)
            ar2_params <- coefs[2] * cos((2 * pi * rep(t_i, n_s) * ar2_scale_params[i]) / n_t - shift_params_space2)
            ar3_params <- coefs[3] * cos((2 * pi * rep(t_i, n_s) * ar3_scale_params[i]) / n_t - shift_params_space3)
            ar1_params_arr[t_i, , i] <- ar1_params
            ar2_params_arr[t_i, , i] <- ar2_params
            ar3_params_arr[t_i, , i] <- ar3_params
        }
        ar_abs_sums <- abs(ar1_params_arr[, , i]) + abs(ar2_params_arr[, , i]) + abs(ar3_params_arr[, , i])
        max_sum <- max(ar_abs_sums)
        ar1_params_arr[, , i] <- ar1_params_arr[, , i] / max_sum
        ar2_params_arr[, , i] <- ar2_params_arr[, , i] / max_sum
        ar3_params_arr[, , i] <- ar3_params_arr[, , i] / max_sum
    }
    ...
    for (i in 1:p) {
        ...
        eps2 <- rnorm(n_s) %*% chol(cov_i)
        eps3 <- rnorm(n_s) %*% chol(cov_i)
        st_data[1, ] <- eps1
        eps_data_i[1, ] <- eps1
        st_data[2, ] <- eps2
        eps_data_i[2, ] <- eps2
        ...
        t_seg <- 1
        for (t_i in 4:n_t) {
            kernel_mat1 <- diag(c(ar1_params_arr[t_i, , i]))
            kernel_mat2 <- diag(c(ar2_params_arr[t_i, , i]))
            kernel_mat3 <- diag(c(ar3_params_arr[t_i, , i]))
            sigma1 <- 1
            eps <- t(rnorm(n_s) %*% chol(sigma1^2 * cov_i))
            eps_data_i[t_i, ] <- eps
            st_data[t_i, ] <-  kernel_mat1 %*% as.matrix(st_data[t_i - 1, ]) + 
                            kernel_mat2 %*% as.matrix(st_data[t_i - 2, ]) +
                            kernel_mat3 %*% as.matrix(st_data[t_i - 3, ]) + eps
        }
        if (add_trend) {
...
save(res_test, file = paste0("res_ar_smooth_sim2/", filename))
res_test
