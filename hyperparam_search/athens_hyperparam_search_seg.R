...
p <- as.integer(args[[1]])
spatial_segment_size <- as.numeric(args[[2]])
temporal_segment_size <- as.numeric(args[[3]])
ar_order <- as.integer(args[[4]])
aux_dim <- as.integer(args[[5]])
file_name <- paste0("pred_seg_", args_unparsed[1], "_", args_unparsed[2], "_", args_unparsed[3], "_", args_unparsed[4], "_", args_unparsed[5], ".RData")
...
results <- data.frame(matrix(NA, nrow = 1, ncol = 6))
names(results) <- c("wMSE", "p", "spatial_segment_size", "temporal_segment_size", "ar_order", "method")

coords_time_shifted <- coords_time_train
coords_time_shifted[, 3] <- coords_time_shifted[, 3] + temp_shift

resiVAEar1 <- iVAEar_segmentation(as.matrix(data_train[, variables]), as.matrix(coords_time_shifted), 
    c(spatial_segment_size, spatial_segment_size, temporal_segment_size), c(1, 1, 2), p, n_s = n_s, 
    seasonal_period = 365, ar_order = ar_order, time_dim = 3,
    aux_hidden_units = c(aux_dim), epochs = 40, batch_size = 64, error_dist_sigma = 0.02, seed = seed,
...
results[1, ] <- c(mean(mse_time_aux/sds^2), p, toString(spatial_segment_size), toString(temporal_segment_size), ar_order, "iVAEar_seg")
