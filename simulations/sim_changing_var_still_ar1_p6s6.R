...
for (j in 1:reps_per_job) {
    ...
    plot(data[seq(1, (n_s*n_t), by = n_s), 5], type = "l")
    plot(trend_i[seq(1, (n_s*n_t), by = n_s)], type = "l")
    plot(as.vector(t(st_data))[seq(1, (n_s*n_t), by = n_s)], type = "l")
...
save(res_test, file = paste0("res_var_blocks/", filename))
res_test
